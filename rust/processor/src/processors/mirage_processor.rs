...
pub struct MirageProcessorConfig {
    ...
    pub market_module_address: String,
}
...
impl MirageProcessor {
    pub fn new(
        ...
        tracing::info!(
            "Initializing MirageProcessor deployer address: {}, mirage address: {}, market address: {}",
            config.deployer_address,
            config.mirage_module_address,
            config.market_module_address
        );
...
pub async fn insert_to_db(
    ...
    for res in [
        cfd_res, vcd_res, vc_res, vd_res, va_res, mcd_res, mc_res, pd_res, tpd_res,
...
impl ProcessorTrait for MirageProcessor {
    ...
    async fn process_transactions(
        ...
        let (
            ...
            current_limit_orders,
            market_activities,
        ) = parse_mirage_protocol(&transactions, &self.config.mirage_module_address, &self.config.market_module_address).await;
...
pub async fn parse_mirage_protocol(
    ...
    let mirage_module_address = standardize_address(&mirage_module_address);
    let market_module_address = standardize_address(&market_module_address);
