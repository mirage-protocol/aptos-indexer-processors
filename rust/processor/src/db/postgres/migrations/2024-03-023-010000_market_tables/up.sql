...
DROP FUNCTION IF EXISTS get_trade_stats(TEXT, TEXT, TEXT);
CREATE OR REPLACE FUNCTION get_trade_stats(
address TEXT,
resolution TEXT,
period TEXT
)
RETURNS SETOF owner_trades_1hour AS $$
SELECT
address::text AS owner_addr,
time_bucket_gapfill(resolution::interval, bucket, NOW() - period::interval, NOW())::timestamptz AS bucket,
